"""Regenerate the FsmGobbletV2 seed=0 golden decision trace.

Run only when FsmGobbletV2's behaviour is intentionally changed:
    ./.venv/bin/python tests/hl_gobblet/_gen_golden_v2.py

Plays a fixed fsm-v2(P0)-vs-random(P1) game at seed 0 and stores the semantic
GobbletTraceRecordV2 fields, so the golden is robust to behaviour-neutral
refactors yet trips on any real change to the controller's move sequence.
"""
...
from hl_gobblet.controllers import FsmGobbletV2  # noqa: E402
...
GOLDEN = Path(__file__).resolve().parent / "golden" / "fsm_gobblet_v2.seed0.json"
SEED = 0


def run_trace(seed: int = SEED) -> list[dict]:
    """Play fsm-v2(P0) vs random(P1) and return the FSM's decision trace as dicts."""
    fsm = FsmGobbletV2()
    fsm.reset(seed)
