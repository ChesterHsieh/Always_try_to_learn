...
> 本堂做 ①–④（單機），⑤–⑧ 在[第四堂](class4_models_to_racks.md)後半的請求路徑裡逐站出現（多機）。前置：第一堂 roofline 與記憶體階層、第二堂多卡平行與互連。
>
...
> **通用判準**：可以快取的是「計算結果」，不可以快取的是「請求狀態」。跨機複製 KV 時也要問同一個問題。
...
硬體對齊：Hopper 有 FP8、Blackwell 有 FP4。**精度格式是硬體規格表上的一行**——[第四堂](class4_models_to_racks.md)前半會看到模型端已經開始「出廠就是 4-bit」。
...
A：單卡不省**容量**（所有專家都要在 HBM），省的是**每 token 的權重讀取量與 FLOPs**。要連容量也省得靠 EP——第四堂的大規模 EP。
