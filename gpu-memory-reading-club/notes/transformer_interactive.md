# 報告 — 玩具級 Transformer 互動地圖：Encoder⟷Decoder 全景 → 計算子 matmul（L2/HBM 搬運）、三種執行模式、GPU/TPU/Groq 對應
...
1. **全景 Encoder ⟷ Decoder**：最宏觀。encoder（雙向、無 mask）讀懂整段來源、壓成一份 **memory M [5×6]**；decoder（masked self + cross + FFN）逐字生成，靠 **cross-attention 把 M 當 K、V** 回看來源。**中間傳的就是這份 M**——decode 時 M 只算一次、每步重用（GPT 這類只留 decoder、把 encoder 拿掉）。點 Encoder block / Decoder block 下鑽。
2. **Block**：依上一層選到的是 encoder 或 decoder block。encoder＝self-attn（雙向）+ FFN；decoder＝masked self-attn → cross-attn（K/V=M）→ FFN，每段後接 Add&Norm。attention 是 token 間唯一交換資訊的地方；FFN 對每個 token 獨立（token 軸天然平行）。
3. **Attention**：X →（×Wq/Wk/Wv）→ Q、K、V [5×6]，6 維「切」成 2 個 head × 3 維。**分叉＝平行軸**：head 之間零依賴 → GPU 上是 batched GEMM / 不同 tile。cross-attn 視圖會標出「Q 來自自己、K/V 來自 encoder memory M」。GQA 就是在這個分叉上動手腳（多個 Q head 共用一組 KV）。
4. **Head**：完整矩陣鏈 Q[5×3]·Kᵀ[3×5] → S[5×5]（self 才打 causal mask；cross 不擋）→ softmax → ·V[5×3] → O[5×3]。**這一層是三種模式差異最清楚的地方**（見下節）。點中間的「·」下鑽到計算子。
5. **計算子（一個 matmul × L2⟷HBM）**：把鏡頭推到單一 matmul（S = Q·Kᵀ）。operands 住在大而慢的 HBM，要算時切 tile 沿 **HBM → L2 → shared/暫存器 → tensor core MMA** 流動，結果再寫回 HBM。**這一層專門回答「L2 與 HBM 怎麼交換資料」**：訓練/prefill 是大 GEMM，tile 在 L2/SRAM 重複用、命中率高 → compute-bound（訓練還要把 S、A 留給 backward → 容量壓力）；decode 是 GEMV，operand 用一次就丟、KV cache 又比 L2 大 → 每步都得從 HBM 重搬、箭頭轉紅 → **L2↔HBM 流量＝瓶頸**、tensor core 空轉。encoder 的每個 matmul 同理。
6. **硬體**：GPU / TPU / Groq 切換（見第 4、5 節）。

操作：點發亮元件下鑽、`Esc` 回上層、`1`–`6` 跳層、`T`/`P`/`D` 切模式；URL hash 可直達（如 `#op.decode.dec.self`、`#head.train.dec.cross`、`#hw.decode.dec.self.tpu`）。
...
## 3b. cross-attention：encoder memory M 到底搬了什麼

這是常被略過、地圖特別補上的一條搬遷（在 Attention/Head/計算子 層切到 cross 即顯示）。**「K、V 來自 encoder memory M」拆成三段**：

1. **encode 一次**：encoder 跑完整段來源 → 算出 memory M `[T_enc×d]` → 寫進 **HBM**。
2. **投影一次（每層）**：每個 decoder 層用自己的 Wk/Wv 把 M 投影成 **cross-K / cross-V** → 也快取在 **HBM**，大小由 T_enc 決定、**固定**。
3. **每步 decode 重讀**：算 cross-attention 時，cross-K/V 從 **HBM → SRAM → tensor core**；每一步讀的都是**同一份**。

**和自回歸（self-attn）KV cache 的關鍵差別**：self-attn 的 KV cache **每生一個字就多一列**（隨生成長度線性長大）；cross 的 K/V **永遠只是「來源那幾列」、不長大**。因此 cross 每步搬運量固定，來源短時甚至可能留在 L2/SRAM（不必每步回 HBM）。實務上 seq2seq 推論引擎（T5、BART、Whisper 等）正是「encode 一次 → cross-KV 預先算好快取 → 逐步解碼重用」。計算子層的 cross 視圖會把 B=Kᵀ 標成「來自 M 的 cross-KV：一次算好、每步重讀（不長大）」，L2 卡也由「裝不下→miss」改為「固定大小、來源短可能留得住」。
...
- 連回合輯：Part 1（HBM/SM/tensor core）、Part 2（roofline、tiling→MMA）、Part 3（prefill/decode/KV cache）、Part 5（FlashAttention 讓 S 不落地 HBM、GQA 讓 KV 變小——都是在減這條串流）。
...
**三種硬體一句話**：GPU 在 HBM 上想辦法（FlashAttention 省 S、GQA 省 KV）、TPU 同樣靠 HBM 但用脈動陣列、Groq 直接把 HBM 換成 SRAM——**記憶體牆是物理，三條路都在繞同一道牆**。

## 6. 建議展示流程（合輯第 25 頁切出來，約 12–16 分鐘）

1. `#io`（訓練模式）：**全景** → encoder 把來源壓成 memory M、decoder 用 cross-attn 回看 M → 「encoder 與 decoder 中間就傳這一份 M」。按 `D` 點出「decode 時 M 只算一次、decoder 逐字生成」。點 **Decoder block** 下鑽。
2. **Block**：decoder 的三段（masked self → cross → FFN）→ 「cross 是回看來源的唯一管道」。點 self-attn（或 cross-attn）切開。
3. 點開 **Attention**：看 6 維切成 2 head 的分叉 → 「分叉＝平行軸」；cross 視圖標出「K/V 來自 encoder M」。
...
- 互動地圖：`interactive/transformer_map.html`（單檔、離線、無相依；6 層：全景 Encoder⟷Decoder → Block → Attention → Head → 計算子 matmul(L2⟷HBM) → 硬體；硬體層三個 tab：GPU / TPU / Groq）。
- 合輯第 25 頁（互動②指引）、第 27 頁（TPU）、第 28 頁（Groq）由 `slides/build/generate_full.js` 產生；記憶體每 GB 價格在第 8 頁。
- 相關講稿：[合輯索引 full_series.md](full_series.md)——prefill/decode/KV cache（Part 3）、Transformer 為平行而生與 FlashAttention/GQA（Part 5）的推導都在其中的「關鍵推導速查」一節（原 s3/s5 單場講稿已併入後刪除）。
