...
- `class6_multi_rack_inference.md` — ✅ **第六堂課（最終章）講稿／索引**：跟著一個 DeepSeek 請求穿過數個機櫃（router → prefill → KV 交接 → decode 熱迴圈），按「多常搬 × 一次多大」把資料分四種、看各自付得起哪條路；合回來指出真瓶頸是 scale-up 域大小，再推論 NVIDIA（HGX／NVL72／Rubin）vs AMD（MI355X／Helios）的條件式選型。主角是 SGLang 96×H100。含 20 頁標題鏈、事實原子強度標註、視覺決策、互動教具的計算模型（§6，以 DeepEP 實測校準）。搭配 20 頁投影片與 [../interactive/rack_journey_map.html](../interactive/rack_journey_map.html)。
