...
- `class3_engine_single_node.md` — ✅ **第三堂課講稿／索引**：推論引擎單機篇，**八個問題 × 兩種解法（SGLang × vLLM）**。那八個問題不是某一家專有的，是任何推論引擎都會撞到的，所以每題都對比兩家的寫法：①程式難平行（SGLang 前端 DSL vs vLLM 沒有對應物）｜②前綴重算（共同：分頁 KV／索引：radix tree vs 鏈式雜湊表／排程：cache-aware 主動 vs 被動）｜③輸出不可控（共同：XGrammar FSM／差異：jump-forward 約 3× 吞吐）｜④CPU 成瓶頸（zero-overhead scheduler vs 多進程+async）｜天花板：投機解碼·MTP、量化。含 27 頁頁面地圖、哲學差異、「四問題 × 兩種寫法」總表、選型判準、Q&A。搭配 [../interactive/serving_map.html](../interactive/serving_map.html)（模式 1–4）。
...
- `class6_multi_rack_inference.md` — 🚧 **第六堂課（最終章）骨架／講稿**：跟著一個 DeepSeek 請求穿過數個機櫃（router → prefill → KV 交接 → decode 熱迴圈），按「多常搬 × 一次多大」把資料分四種、看各自付得起哪條路；合回來指出真瓶頸是 scale-up 域大小，再推論 NVIDIA（HGX／NVL72／Rubin）vs AMD（MI355X／Helios）的條件式選型。主角是 SGLang 96×H100。含 20 頁標題鏈、事實原子強度標註、視覺決策、互動教具的計算模型（§6，以 DeepEP 實測校準）。搭配 [../interactive/rack_journey_map.html](../interactive/rack_journey_map.html)。**投影片待標題鏈確認後產出。**
