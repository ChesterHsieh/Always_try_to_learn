...
# hl-lunar-lander Specification

## Purpose

在 `LunarLander-v3` 上以 HL（啟發式學習）典範執行的最小框架，包含 policy 介面、env adapter、runner、baseline procedural controller、弱對照組與實驗紀錄結構，作為後續所有 `hl-*` 子主題可直接 fork 的「最小可跑」骨架。
